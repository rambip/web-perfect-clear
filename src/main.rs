...
// TODO: definir les positions relatives de pieces possibles
...
fn liste_perfect_clear(etat: &mut EtatJeu, acc: &mut Vec<Vec<u8>>) {
    if etat.pieces_jouees.len() == 10 {
        if etat.couleur_jouees.iter().all(|&x| x>=1) {
...
impl Component for Model {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        let slider_value = |e: InputEvent| e.target_unchecked_into::<HtmlInputElement>().value_as_number() as f32;
